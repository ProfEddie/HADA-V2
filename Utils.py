...
class Retrieval_Dataset(Dataset):
    def __init__(self, config, dataset, model_1_dict, model_2_dict, type_dataset='train'):  
    ...
    def set_branch(self, branch='img'):
        ...
        if self.type_dataset == 'train':
            ...
            if self.branch in ['img', 'image']:
                self.dataset = self.ori_dataset[self.ori_type_dataset]
            else: # text -- txt -- cap
                t = [x['caption'][:5] for x in self.ori_dataset[self.ori_type_dataset].annotation]
                t = list(itertools.chain.from_iterable(t))
                # self.dataset = self.ori_dataset[self.ori_type_dataset].text # list of text (already ordered)
