...
class Controller(nn.Module):
    def __init__(self, config):
        ...
        # MODEL
        self.img_encoder =  Poincare_MH(self.manifold, f1_in=self.f1_in, f2_in=self.f2_in, f1_out=self.f1_out, f2_out=self.f2_out,
        ...
        self.cap_encoder = Poincare_MH(self.manifold, f1_in=self.f1_in, f2_in=self.f2_in, f1_out=self.f1_out, f2_out=self.f2_out,
                              ft_trans=self.ft_trans, ft_gcn=self.ft_gcn, ft_com=self.ft_com,
                              type_gcn=self.type_gcn, skip=self.skip,
                              batch_norm=self.batch_norm, dropout=self.dropout, act_func=self.act_func)
        self.discriminator = Poincare_Discriminator(self.manifold, ft_in=self.ft_com[-1], ft_out=self.ft_itm,
                                           batch_norm=self.batch_norm, dropout=self.dropout, act_func=self.act_func)
        self.img_encoder_m = Poincare_MH(self.manifold, f1_in=self.f1_in, f2_in=self.f2_in, f1_out=self.f1_out, f2_out=self.f2_out,
                                ft_trans=self.ft_trans, ft_gcn=self.ft_gcn, ft_com=self.ft_com,
                                type_gcn=self.type_gcn, skip=self.skip,
                                batch_norm=self.batch_norm, dropout=self.dropout, act_func=self.act_func)
        self.cap_encoder_m = Poincare_MH(self.manifold, f1_in=self.f1_in, f2_in=self.f2_in, f1_out=self.f1_out, f2_out=self.f2_out,
                                ft_trans=self.ft_trans, ft_gcn=self.ft_gcn, ft_com=self.ft_com,
                                type_gcn=self.type_gcn, skip=self.skip,
                                batch_norm=self.batch_norm, dropout=self.dropout, act_func=self.act_func)
